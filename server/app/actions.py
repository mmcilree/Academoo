...
def getFilteredPosts(limit, community_id, min_date):
    ...
    post_dicts = [{"id": post.id, "parent": post.parent_id, "children": [comment.id for comment in post.comments], "title": post.title, "contentType": post.content_type, "body": post.body, "author": {"id": post.author.id, "host": post.author.host}, "modified": post.modified, "created": post.created} for post in posts]
    return post_dicts
...
def getPost(post_id):
    ...
    if is_comment:
        post_dict = {"id": post.id, "parent": post.parent_id, "children": [comment.id for comment in post.comments], "title": post.title, "contentType": post.content_type, "body": post.body, "author": {"id": post.author.id, "host": post.author.host}, "modified": post.modified, "created": post.created}
    else:
        post_dict = {"id": post.id, "parent": post.community_id, "children": [comment.id for comment in post.comments], "title": post.title, "contentType": post.content_type, "body": post.body, "author": {"id": post.author.id, "host": post.author.host}, "modified": post.modified, "created": post.created}
